  ...
  return (
    <div className="pt-28 pb-10 px-8 mx-auto max-w-screen-2xl min-h-[80vh]">
      ...
      <div className="bg-white border border-blue-200 rounded-lg dark:bg-gray-800 dark:border-blue-700 shadow-md shadow-blue-200 dark:shadow-blue-500 p-6 min-h-[55vh]">
        <div className="grid grid-cols-4 justify-between px-5 pb-3 gap-5 text-gray-900 dark:text-white text-lg font-bold">
          <p>NAME</p>
        ...
        <div className="space-y-4">
          {leaderBoardData?.data?.length === 0 && (
            <p className="text-lg font-semibold text-center dark:text-gray-300 mt-20">
              There is no performer in this category
            </p>
          )}
          {leaderBoardData?.data?.length > 0 &&
            leaderBoardData?.data?.map((lbd: any, index: any) => (
              <li
                key={index}
                className="grid grid-cols-4 items-center gap-5 px-5 py-3 bg-slate-100 shadow rounded-lg border border-slate-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600"
              >
                <div className="flex items-center gap-5">
                  ...
                  <Image
                    ...
                    src={
                      lbd?.user?.profileImg ? lbd?.user?.profileImg : Profile
                    }
                    alt="title"
                    priority={true}
                    quality={100}
